...
vector<uint32_t>sm3_hash()
{
	...
	unsigned char buffer[4] = { 0x01,0x23,0x45,0x67 };  // assign input
...
vector<uint32_t>sm3_hash_optimized()
{
	...
	vector<uint32_t> hash_result(32, 0);  // (0, 0, ..., 0), 32 zeros
	unsigned char buffer[4] = { 0x01,0x23,0x45,0x67 };  // input is same with ordinary version
	unsigned char hash_output[32];
...
int main() {
	...
	duration<double, ratio<1, 1000>> diff1 = (end1 - start1) / 1000;
	cout << diff1.count() << " ms\n";
	...
	duration<double, ratio<1, 1000>> diff2 = (end2 - start2) / 1000;
	cout << diff2.count() << " ms\n";
